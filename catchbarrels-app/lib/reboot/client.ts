export class RebootClient {
    ...
    async exportSwingData(sessionId: string, orgPlayerId: string, movementTypeId: number = 1, dataType: string = "hitting-processed-metrics", dataFormat: string = "csv") {
        ...
        if (contentType && contentType.includes('application/json')) {
            ...
            if (json.download_urls && json.download_urls.length > 0) {
                console.log(`[RebootClient] Fetching CSV from URL: ${json.download_urls[0]}`);
                const csvRes = await fetch(json.download_urls[0]);
                if (!csvRes.ok) throw new Error('Failed to fetch CSV from download URL');
                csvText = await csvRes.text();
            } else {
                // Unexpected JSON format or no URLs
                console.warn('[RebootClient] Unexpected JSON response:', json);
                return { metrics: {} };
            }
        } else {
            csvText = await response.text();
        }
        console.log('[DEBUG] Raw CSV Response:', csvText);
        // Parse CSV to JSON
        ...
        headers.forEach((header, index) => {
            // Clean header name: remove quotes and whitespace
            const key = header.trim().replace(/^"|"$/g, '');
            let value = values[index] ? values[index].trim().replace(/^"|"$/g, '') : null;
    ...
    async listSessions(orgPlayerId: string, limit = 20, movementTypeId?: number) {
        ...
        // Reboot API returns an array of sessions directly
        if (Array.isArray(data)) {
            return {
                sessions: data.map((s: any) => ({
                    ...s,
                    session_id: s.id || s.session_id // Normalize to session_id
                }))
            };
        }
    ...
    async searchPlayers(query: string) {
        // Reboot API doesn't seem to support search, so we fetch recent players and filter locally.
        // This is a limitation, but sufficient for finding recent athletes.
        const allPlayers = await this.listPlayers(100); // Fetch last 100

        if (!Array.isArray(allPlayers)) {
            console.warn('Expected array from listPlayers, got:', typeof allPlayers);
            return [];
        }
